"""Encapsulate AVR remote control commands."""
...
Bytes2Command = {v: k for k, v in Command2Bytes.items()}


def decode(quad):
    """Parse the 4 bytes containing a command sent to the AVR.

    The AVR receives 4-byte datagrams over the serial port (not including
    the protocol overhead managed by AVR_Connection) containing remote
    control commands to be executed by the AVR. Return the command keyword
    corresponding to the given 4-byte data value, or raise a KeyError if no
    corresponding keyword is found.
    """
    if not isinstance(quad, bytes):
        raise TypeError('data must be a bytes object')
    if not len(quad) == 4:
        raise ValueError(f'Unexpected data length {len(quad)}')

    return Bytes2Command[quad]


def encode(command):
    return Command2Bytes[command]
